		...
		for (int m = 2; m <= n; m *= 2)
		{
			...
			for (int k = 0; k < n; k += m)
			{
				...
				for (int j = 0; j < m / 2; j++)
				{
					...
					double tImag = (xReal * reversedImag[k + j + m / 2]) + (xImag * reversedImag[k + j + m / 2]);

					double uReal = reversedReal[k + j];
					double uImag = reversedImag[k + j];

					reversedReal[k + j] = tReal + uReal;
					reversedImag[k + j] = tImag + uImag;

					reversedReal[k + j + m / 2] = tReal - uReal;
					reversedImag[k + j + m / 2] = tImag - uImag;
